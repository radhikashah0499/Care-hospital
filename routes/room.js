...
router.get('/app/addroomPage', (req, res) => {
    res.status(200).render('addroom', {pageTitle: "Add room"});
});
...
router.post('/app/addroomPage', (req, res) => {
    ...
    if (_.isString(roomName) && !_.isNaN(roomName)) {
        ...
        room.save().then((room) => {
            ...
            res.status(200).redirect('/app/addroomPage');
        }).catch((err) => {
            console.log(err);
            res.status(400).redirect('/app/addroomPage');
        });
    } else {
        res.status(400).redirect('/app/addroomPage', {messages: req.flash('success_msg', 'Room added succesfully.') });
    }
});
